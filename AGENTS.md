...
Timepiece Studio is a system of native desktop objects. Clock and Photo are the first two objects.
...
Interaction quality matters more than feature count. Never trade away Hide/Fade behavior, true transparency, native movement, persistence, recovery, or idle performance merely to add features.
...
Floating Clock + Photo Windows (`index.html?window=clock|photo`)
...
- `src/main.js`: Home, Objects, Clock Faces, app Settings, object inspectors, native event listeners, edit controls, and face selection.
...
- `src-tauri/capabilities/default.json`: Tauri permissions for `main`, `clock`, and `photo`.
- `public/assets/`: bundled production clock faces; every face exposed in the gallery must have real alpha transparency.
...
The current runtime owns one Clock and one Photo. Keep behavior conceptually generic:

```text
DesktopObject + Behaviour
```

Examples include `Photo + Hide`, `Clock + Fade`, and future `Note + DoNothing`. Do not create clock-specific behavior forks when the generic engine can express the rule.
...
- Independent visibility behavior (`Hide`, `Fade`, or `Do nothing`) and pointer behavior (`Let clicks pass through` on/off).
...
## The core Hide-on-hover rule

Never implement Hide on hover using only `mouseenter`, `mouseleave`, `pointerenter`, `pointerleave`, or CSS `:hover`. Once an object becomes click-through, browser pointer events can stop. `WebviewWindow::cursor_position()` plus native window bounds and the explicit Rust state machine are part of the product architecture.

Hide is not complete because a control exists. It is complete only when the native window disappears, becomes click-through, stays out while the cursor remains in its original bounds, lets the underlying application receive input, and returns after exit without flicker. Persistent click-through is a separate setting and must compose with every visibility behavior.
...
Permanent Hide-on-hover regression:
...
- The native behavior loop currently polls around 30 Hz only when Hide/Fade/Edit needs it and slows for idle modes. Measure before changing it.
